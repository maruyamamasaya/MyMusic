const state={dashboardPeriod:'7d',tracksPeriod:'all',tracksSort:'playCount',tracksOrder:'desc',tracksPage:1,sourceKind:'track_features',sourcesPage:1};
...
function renderBars(sel,rows){if(!rows.length){$(sel).innerHTML=empty();return}const max=Math.max(...rows.map(x=>x.value),1);$(sel).innerHTML=rows.map(x=>`<div class="bar-item"><div class="bar" style="--height:${Math.max(4,x.value/max*100)}%" data-tip="${esc(x.label)}: ${x.value}回"></div><span>${esc(x.label.slice(5).replace('-','/'))}</span></div>`).join('')}
...
function renderRanks(sel,rows,labelOnly=false){$(sel).innerHTML=rows.length?rows.map((x,i)=>`<div class="rank-row"><span class="rank">${i+1}</span><div><strong title="${esc(labelOnly?x.label:x.title)}">${esc(labelOnly?x.label:x.title)}</strong>${labelOnly?'':`<small title="${esc(x.artist)}">${esc(x.artist)}</small>`}</div><span>${x.value}${labelOnly?'曲':'回'}</span></div>`).join(''):empty()}
...
async function loadSources(){
  const d=await api(`/api/sources/${state.sourceKind}?page=${state.sourcesPage}`),trackKind=['track_features','volume_normalization'].includes(state.sourceKind);
...
$$('.periods button').forEach(b=>b.onclick=()=>{const group=b.closest('.periods');group.querySelectorAll('button').forEach(x=>x.classList.remove('active'));b.classList.add('active');if(group.dataset.target==='dashboard'){state.dashboardPeriod=b.dataset.period;loadDashboard()}else{state.tracksPeriod=b.dataset.period;state.tracksPage=1;$('#custom-period').hidden=b.dataset.period!=='custom';loadTracks()}});
