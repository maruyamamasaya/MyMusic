...
async function loadTracks(){
  ...
  $('#tracks-body').innerHTML=d.tracks.length?d.tracks.map(x=>`<tr>
    ...
    <td>${preference(x.playbackPreference)}</td><td><span class="favorite">${x.favorite===1?'♥':'—'}</span></td>
    <td>${x.playCount}回</td><td>${duration(x.totalPlayTime)}</td><td>${percent(x.completionRate)}</td><td>${percent(x.skipRate)}</td>
    <td>${x.lastPlayedAt?new Date(x.lastPlayedAt).toLocaleString('ja-JP'):'—'}</td></tr>`).join(''):`<tr><td colspan="8">${empty()}</td></tr>`;
}
