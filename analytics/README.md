...
- Insights: `play_source`別、`selection_type`別、および両者の組み合わせごとに再生回数、総再生時間、完走率、Skip率、Early Skip回数・率を表示。値は固定enumではなくImport済みイベントから動的に列挙。「分析可能データのみ」（既定）と「すべて」の品質フィルターを期間指定と併用可能。音楽特徴と再生行動ではDark／Calm／Aggressive／Piano／Electronic／Ambient／Drum & Bass／Vocal／Instrumentalを5段階に分け、対象曲数・再生回数と率を比較表示
...
主なAPIは`GET /api/dashboard?period=7d`、`GET /api/music-history`、`GET /api/insights?period=7d&quality=analyzable`、`GET /api/insights/features?feature=dark&period=7d&quality=analyzable`、`GET /api/rankings`、`GET /api/tracks`、`POST /api/import`、`GET /api/imports`、`PUT /api/preferences/{track_id}`、`GET /api/preferences/export`です。Insightsの`quality`は`analyzable`（既定）または`all`です。特徴量APIは現在の全Track Feature中で最新の数値`analysisVersion`だけを使用し、レスポンスにもVersionを明示します。FastAPIのAPI仕様は起動中の`/docs`で確認できます。
