...
- 通常shuffle、Quick Play、Favorite系shuffle、Repeat、Selective Random候補、Genre Randomの選択曲より後、PlayerStoreの自動shuffle order、ハイライトの曲順は、1選曲単位の同じOverplay snapshotを使う。ハイライトは`PlaybackHistoryStore`の通常shuffle eligibilityとPreference × Overplay基本weightだけを共有し、その後の順位付けを`HighlightSelectionPolicy`へ閉じる。アガる／穏やか／発掘は適合度を0.05幅のbandにして第一sort keyとし、同じband内だけPreference × Overplay × Recent Highlightを第二sort keyにする。直近Highlight再生は最新20件を0.20倍、21〜50件を0.60倍にし、同一曲は最新位置を使う。Randomは第二sort keyへの±0.5% jitterだけで、異なる適合度bandを逆転しない。シャッフルは適合度を使わず第二keyから順位付けする。特徴量欠損は適合度0.5の中立とし、Boredom中／永久shuffle非表示だけは全モードから除外する。未再生Discoveryと作業用再生は各入口の目的を維持するためPreferenceだけを使う。手動選択にも適用しない。
