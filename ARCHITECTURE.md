...
起動時は`LibraryStore`が登録済みfolderをまとめて`LibraryPersistenceService`へ渡し、Application Supportの`library-index.json`を1回だけread／decodeする。cache schema v2はfolder pathとTrack配列だけを保持し、Album／Artist／Genre／Composerは復元したTrackから再構築する。旧schemaの完成`MusicLibrary` snapshotと単一folder snapshotはread互換を維持し、次回保存時にv2へ移行する。復元TrackのIdentity登録はcache内の軽量属性だけを使い、起動時に各音源のresource valuesを読み直さない。Identity registryは実際に差分がある場合だけ保存する。cacheは再scan可能な派生データであり、Track Identityやユーザーデータの正本にはしない。
