...
- `LibraryCleanupCandidateService` はlibraryと履歴snapshotを読み、既存日別集計のearly skip合計が3以上かつmanual playが1以上の曲だけを表示用候補へ変換する。評価値は判定に使わず、early skip降順・同数時は最終再生の新しい順に並べる純粋な読取責務であり、履歴・評価・飽き度・shuffle非表示を変更しない。
- `PlaybackPreferenceWeightPolicy` はGood / Bad（-10〜+10）を正の選曲重みへ写像する唯一の責務を持つ。`PlaybackBehaviorAnalyzer`はlibraryと保存済み日別集計のsnapshotから`OverplayScoring`（直近7日／その前56日）と`PreferenceDriftScoring`（直近30日／それ以前、historical最低8件）を呼び、分析結果と候補を純粋に導出する。スコアはSQLiteへ保存せず、Good / Bad、飽き度、恒久非表示を変更しない。M3ではOverplayをshuffle、Quick Play、Auto Queue、Stationへ接続しない。
