...
- `LibraryCleanupCandidateService` はlibraryと履歴snapshotを読み、既存日別集計のearly skip合計が3以上かつ直接選択による開始eventが1以上の曲だけを表示用候補へ変換する。PlayerStoreは「次へ／前へ」で選ばれた移動先を`user_advanced`として直接選択と分離する。event分類導入前でeventを持たない履歴だけは従来のmanual累計を互換fallbackに使う。評価値は判定に使わず、early skip降順・同数時は最終再生の新しい順に並べる純粋な読取責務であり、履歴・評価・飽き度・shuffle非表示を変更しない。
