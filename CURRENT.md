...
- ホームのタイトルをインライン表示にし、同じ行の右側へ「今日 N回 · X分／X時間Y分」を小さく表示する。回数はローカル日付で記録した再生開始回数、時間は同日に開始して終了済みの再生イベントの実聴秒数を合算する。再生開始・終了と日付変更で更新するため、再生中セッションの時間は終了時に反映される。

### MIX Beta

- ホームのプレイリスト直下にMIXを追加。Daily Mix、Rediscovery Mix、My Favorites Mixの3タイルはタップで一時キューを先頭から即再生し、先頭曲のアートワークを表示する。画像のない先頭曲は既定のグラデーションを表示する。
- Dailyは最近30日に聴いた曲、Favorite／Good、再生0〜1回の曲、60日以上聴いていない再生実績のある曲を交互に採り、足りなければ通常シャッフル対象から補充する。Rediscoveryは累計3回以上かつ最終再生が60日以上前の曲、My FavoritesはFavoriteまたはGoodが正の曲を対象とする。各キューは最大25曲で、同じローカル日付・データなら順序が安定する。
- 通常シャッフルと同じ短い曲・作業用BGM・非表示曲の除外とPreference／Overplay重みを適用する。該当曲がないMIXはタイルを無効化する。実際のユーザーデータでの選曲と見た目は未確認。

### 最近の音楽傾向 Beta

- ホームのアクティビティに「最近の音楽傾向」を追加。専用画面で6時間、24時間、3日、7日、30日、3か月、6か月、1年を切り替え、既存Track Featureと完了した再生イベントの時系列平均を1特徴ずつ表示する。総合には十分な再生数・曲数と10ポイント以上の差がある特徴のみを表示し、30日以上は期間の前半・後半の数値も比較できる。Acousticは現行Track Featureに存在しないため対象外。
- 直近1年の対象イベントを画面表示時に一度だけ軽量化し、期間変更時の集計はバックグラウンドで行う。履歴不足・Feature未取得時はEmpty Stateを表示する。実際のユーザーデータでの見た目は未確認。
...
- **日常の音楽史カード**: 最新年の音楽史トップに「今日の音楽史」を追加。再生イベントから1年前の今日、今月の1曲／新しい曲／Artist、再発見、長期・複数月の曲、夜の曲、自動再生から手動選択へ移った曲、最多再生日、今月の音、Albumの12種を候補として作り、成立条件と主役曲の重複を考慮して最大8件を表示する。カード全体のタップで、代表曲を先頭にカード条件に合う曲を最大10曲の一時Queueとして再生する。再生直前に現在Libraryとファイルの読み取り可否を確認し、存在しない曲は飛ばす。Feature欠損や開始経路不明から推測しない。既存の年表示、ランキング、カレンダー、Time Capsuleを維持する。実機での表示と大量履歴時の体感は未確認。
...
- **気分ステーション再チューニング**: 年代指定を撤去し、質問を「気分」と「特に聴きたい音の要素」の2問へ整理した。Semantic v2のraw headを確率として固定閾値へ直接当てず、現在の対象Library内percentileへ正規化してから気分profileとの近さを評価する。差が小さすぎる軸はscoreに使わず、1問目は実際に候補を作れる気分、2問目はvocal／instrumental／electronic／ambient／pianoのうち十分な件数と差がある選択肢だけを表示する。「指定しない」では気分だけ、「任せたい」＋「指定しない」では解析済み対象曲全体から選ぶ。
