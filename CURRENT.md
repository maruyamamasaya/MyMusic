...
- **日常の音楽史カード**: 最新年の音楽史トップに「今日の音楽史」を追加。再生イベントから1年前の今日、今月の1曲／新しい曲／Artist、再発見、長期・複数月の曲、夜の曲、自動再生から手動選択へ移った曲、最多再生日、今月の音、Albumの12種を候補として作り、成立条件と主役曲の重複を考慮して最大8件を表示する。現在Libraryにない曲は候補から外し、Feature欠損や開始経路不明から推測しない。既存の年表示、ランキング、カレンダー、Time Capsuleを維持する。実機での表示と大量履歴時の体感は未確認。
